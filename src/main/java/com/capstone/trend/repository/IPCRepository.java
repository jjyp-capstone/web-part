...
public interface IPCRepository extends JpaRepository<IPC, Integer> {
    ...
    @Query(value = "select * from (select * from ipc_list order by frequency desc limit 10) as A order by ipc_code", nativeQuery = true)
    List<IPC> find_top10();

    @Query(value = "select * from (select * from ipc_list order by average_fluctuation_rate desc limit 10) as A order by ipc_code", nativeQuery = true)
